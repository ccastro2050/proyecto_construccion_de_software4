...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8045
EXPOSE 8045
...
| `EXPOSE 8045` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    CLIENTE["Cliente de base de datos<br/>(DBeaver, SQLTools, psql, sqlcmd)"]
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8045"]
    ...
    NAV -->|"localhost:8045"| APIFACTURAS
    ...
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15445"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11445"| SQLSERVER
```
...
```yaml
  postgres:
    ...
    ports:
      - "15445:5432"                 # "puerto en su PC : puerto interno"
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8045`, `localhost:15445`); entre contenedores, nombres de
...
docker network create proyecto_construccion_de_software4_default

# 2. postgres
docker run -d --name postgres --network proyecto_construccion_de_software4_default --restart unless-stopped `
  ...
  -p 15445:5432 postgres:16-alpine

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. sqlserver
docker run -d --name sqlserver --network proyecto_construccion_de_software4_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Construccion123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11445:1433 mcr.microsoft.com/mssql/server:2022-latest

# 5. ESPERAR a que responda de verdad… mirándolo a ojo

# 6. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_construccion_de_software4_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Construccion123!" `
  -v "${PWD}/db:/scripts:ro" mcr.microsoft.com/mssql/server:2022-latest /scripts/init_sqlserver.sh

# 7. Construir la imagen de api-facturas y encenderla
docker build -t api-facturas ./api_facturas
docker run -d --name api-facturas --network proyecto_construccion_de_software4_default --restart unless-stopped `
...
```yaml
URL_API: http://api-facturas:8045
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8045` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8045` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8045:8045`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8045`, el front
...
```powershell
docker network inspect proyecto_construccion_de_software4_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_construccion_de_software4` produce
la red `proyecto_construccion_de_software4_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_construccion_de_software4-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_construccion_de_software4_default has active endpoints (name:"proyecto_construccion_de_software4-api-facturas-1" …)
```
